	...
	async forceInitDevices() {
		...
		try {
			...
			if (this.deviceAccessDeniedError) {
				...
				this._isVideoMuted =this.libSrv.videoMuted.getValue() === undefined? this.storageSrv.isVideoMuted(): Boolean(this.libSrv.videoMuted.getValue());
				this._isAudioMuted =this.libSrv.audioMuted.getValue() === undefined? this.storageSrv.isAudioMuted(): Boolean(this.libSrv.audioMuted.getValue());
