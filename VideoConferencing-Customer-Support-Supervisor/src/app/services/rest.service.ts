...
export class RestService {
	...
	async getTokensFromBackend1(sessionId: string ,notifyTo: string): Promise<SessionResponse> {
		try {
			return lastValueFrom(this.http.post<any>(this.baseHref + 'sessions', { sessionId , notifyTo }));
		} catch (error) {
			if (error.status === 404) {
				throw { status: error.status, message: 'Cannot connect with backend. ' + error.url + ' not found' };
			}
	...
	async postRequest(path: string, body: any, options?: any): Promise<any> {
		try {
			return lastValueFrom(this.http.post<any>('http://172.17.0.122:8080/sessions', body, options));
		} catch (error) {
			if (error.status === 404) {
				throw { status: error.status, message: 'Cannot connect with backend. ' + error.url + ' not found' };
			}
